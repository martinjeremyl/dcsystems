...
export class AuthenticationService {
  ...
  public login(credentials: Credential) {
    return this.http.post<TokenResponse>(
      `${environment.urlApi}login_check`,
      credentials
    );
  }
  ...
  public resetPassword(email: string) {
    return this.http.post<ResponseApi>(
      `${environment.urlApi}user/reset-password`,
      email
    );
  }
  ...
  public getUserInfo(): void {
    if (this.isUserSignedIn()) {
      this.http
        .get<ResponseApi>(`${environment.urlApi}user`)
        .subscribe((response) => {
  ...
  public moreInformations(user: User): Observable<ResponseApi> {
    return this.http.put<ResponseApi>(
      `${environment.urlApi}user/update-main-informations`,
      user
    );
  }
  ...
  public getAvatar() {
    return this.http.get(`${environment.urlApi}user/avatar`, {
      responseType: 'blob',
    });
  }
}
